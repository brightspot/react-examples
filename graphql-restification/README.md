...
`POST_BRIGHTSPOT` function located in `src/api` has the following code to make a call that will send a POST request to return the data requested:
...
## WelcomeContainer functional component

The WelcomeContainer has the following functions:
...
`getArticle` will trigger on click of the button. It will check whether `data?.isClicked` is set to false, if it is, it should change to true and get the Brightspot article. If is set to true, it should change to false and return the Hello World article.
...
`handleError` is being used to set any error messages returned from the requests made:
...
`useEffect` is being used here on page load to make get request using `getHelloWorldArticle` to display the Hello World article made in this example or if it is unable to successfully retrieve the data, it will receive errors to display.
