...
const AppView: NextPage<Props> = ({ app }) => {
  ...
  const randPageIndex = Math.floor(Math.random() * (app.Page_app_connection?.items.length ? app.Page_app_connection.items.length : 0))
  ...
  allArticles.sort((a, b) => b.cms_content?.publishDate - a.cms_content?.publishDate)

  return (
    <div>
      <Navbar pages={app.Page_app_connection?.items ? app.Page_app_connection.items : []}/>
      <Container>
